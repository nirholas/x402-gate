...
/**
 * @title DeployX402Gate
 * @notice Deploys X402Gate deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployX402Gate is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(X402GateHook).creationCode;
        ...
        console2.log("X402Gate ", predicted);
        ...
        vm.startBroadcast();
        X402GateHook hook = new X402GateHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("X402Gate")),
